...
- [x] **T14** — **Netlify deployment validation**: add a `netlify.toml` `[build]` section with `command = "npm run build"` and `publish = ".next"`, verify scheduled function cron syntax matches Netlify format
- [x] **T15** — **Update README** with correct deployment steps, env var table, and actual Supabase migration order
